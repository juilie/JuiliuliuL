...
function setup() {
  ...
  ROTATION_RANGE_SLIDER = createSlider(0, 1000, 20).parent(createDiv('Rotate Range'));
  ROTATION_MODIFIER_SLIDER = createSlider(0, 360, 0).parent(createDiv('Rotate Mod'));

  EGG_MODE = createCheckbox('Egg Mode', false);

  THREE_D_MODE = createCheckbox('3D Mode', true);

  SPINNING = createCheckbox('Spinning (3D Only)', true);
  TREE_SWITCHING = createCheckbox('Tree Switching (2D only)', false);

  ROTATION_WAVE = createCheckbox('Rotation Wave', false);


  HIDE_BRANCHES = createCheckbox('Hide Branches', false);
  SWAY_MODE = createCheckbox('Sway Mode (2D Only)', false);
  SWAY_MODE_SLIDER = createSlider(0, 5, 2).parent(createDiv('Sway Amount'));

  LEAF_SIZE = createSlider(0, 180, 0).parent(createDiv('Leaf Size'));
  LEAF_SIZE_Y = createSlider(0, 180, 0).parent(createDiv('Leaf Length'));
  LEAF_SIZE_X = createSlider(0, 180, 0).parent(createDiv('Leaf Width'));
  LEAF_SIZE_Z = createSlider(0, 180, 0).parent(createDiv('Leaf Depth'));

  LEAF_COLOR = color('green')
  LEAF_COLOR_PICKER = createColorPicker(color('green')).parent(createDiv('Leaf Color'));

  LEAF_OUTLINE_PICKER = createColorPicker(color(70, 40, 20)).parent(createDiv('Leaf Outline Color'));
  const leafShapeWrapper = createDiv()
  leafShapeWrapper.id("leafShapeWrapper")
  createP("Shape").style('text-decoration', 'underline');
  LEAF_SHAPE = createRadio(leafShapeWrapper);
  ...
  BRANCH_COLOR_PICKER = createColorPicker(color('brown')).parent(createDiv('Branch Color'))
  BRANCH_RADIUS = createSlider(0, 180, 0).parent(createDiv('Branch Radius'));
  BRANCH_LENGTH = createSlider(-180, 180, 0).parent(createDiv('Branch Length'));
  ...
  createP("Texture").style('text-decoration', 'underline');
  BRANCH_TEXTURE = createRadio(textureWrapper);
  ...
  createP("Shape").style('text-decoration', 'underline');
  BRANCH_SHAPE = createRadio(shapeWrapper);
  ...
  CAMERA_X = createSlider(-width / 2, width / 2, 0).parent(createDiv('X'));
  CAMERA_Y = createSlider(-height / 2, height / 2, 0).parent(createDiv('Y'));
  CAMERA_Z = createSlider(-width / 2, width / 2, 0).parent(createDiv('Z'));

  // EXPORT_SECONDS.position(20, 65);

  let valueDisplayer = createP("5 second(s)");
  EXPORT_SECONDS = createSlider(1, 60, 4).parent(createDiv('Seconds'));
  EXPORT_SECONDS.input(() => {
    valueDisplayer.html(EXPORT_SECONDS.value() + " second(s)")
  })
  SAVE_BUTTON = createButton('Save to gif', '5', 'number');
  ...
  RANDOMIZE_BUTTON = createButton('Randomize', '5', 'number');
...
function draw() {
  ...
  if (THREE_D_MODE.checked()) {
    ...
    SPINNING.checked() && rotateY(frameCount / 8)
